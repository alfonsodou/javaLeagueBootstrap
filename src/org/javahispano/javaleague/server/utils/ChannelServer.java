...
public class ChannelServer {
	...
	private static final String APP_KEY = "JavaLeague-";
	...
	private static boolean setChannelAPIEnabled() {
		String skey = props
				.getProperty("com.metadot.connectr.enable-channelapi");
		if (skey != null) {
	...
	public static void pushMessage(User user, Message msg) {
		if (user.getChannelId() == null) {
			logger.log(
					Level.SEVERE,
					"Can't push a message to a null channeID. Maybe the Channel API is disabled in javaLeague.");
			return;
	...
	public static String createChannel(String userUniqueId) {
		if (!USE_CHANNEL_API) {
			logger.log(Level.WARNING,
					"Channel API disabled in appengine-web.xml. Not creating.");
			return null;
		}
